...
var parserUrlEncoded = naan.curry(express.urlencoded, { strict: false, extended: true });
...
module.exports = function(model, opts) {
  ...
  controller.use(parserJson());
  controller.use(parserUrlEncoded());
  ...
  controller.router.param(model.type, controller.param);
  ...
  controller.middleware('mutators', function(req, res, next) {
    ...
    if (!req.headers['content-type'].match(/application\/json/i))
      return res.status(415).json("Data must be sent as application/json");
    next();
  });
  ...
  controller.define(
    ...
    function(req, res, next) {
      if (req[model.type] === false) return res.sendStatus(404);
  ...
  controller.define(
    ...
    function(req, res) {
      model.save(req.body, function(err, node) {
        if (err)  return res.status(err.statusCode || 500).json(err.message || err);
        else res.status(201).json(node);
      });
    });
  ...
  controller.define(
    ...
    function(req, res) {
      ...
      model.save(req.body, function(err, node) {
        if (err) res.status(err.statusCode || 500).json(err.message || err);
        else res.json(node);
      });
    });
  ...
  if (Array.isArray(model.fields)) {
    model.fields.forEach(function(field) {
      controller.define('read:' + field, 
        ...
        function(req, res) {
          req.params._id = parseInt(req.params._id, 10);
          if (isNaN(req.params._id)) return res.status(400).json("Invalid ID");
  ...
  Object.keys(model.compositions).forEach(function(comp) {
    ...
    controller.define('read:' + comp.name,
      [ 'compositions', 
        checkAccess('r', '_id') ],
      function(req, res) {
        if (req.params._id == null) return res.status(400).json("Invalid ID");
