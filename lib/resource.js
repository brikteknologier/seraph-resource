...
module.exports = function(model, routes = {}) {
  ...
  var routes = _.extend({
    'create': 'POST ' + root
    'read': 'GET ' + join(root, param),
    'update': 'PUT ' + join(root, ':id?'),
    'delete': 'DELETE ' + join(root, ':id?'),
    'rel:read': 'GET ' + join(root, ':_id', 'rel', ':_type', ':_direction?')
    'rel:create': 'POST ' +join(root, ':_from', 'rel', ':_type', ':_to')
    'create:property': 'POST ' + join(root, param, ':property')
    'read:property': 'GET ' + join(root, param, ':property')
    'update:property': 'PUT ' + join(root, param, ':property')
    'delete:property': 'DELETE ' + join(root, param, ':property')
  }, routes);

  getRoute = function(action, property) {
    if (property != null) {
      action += ':property';
    }
