...
module.exports = function(model) {
  ...
  function(req, res) {
    ...
    if (req.params.type == null || isNaN(req.params.id)) {
      return req.send(400);
    }
    req.params.direction = req.params.direction || 'all';
    model.db.relationships(req.params.id, 
      req.params.direction, req.params.type, function(err, rels) {
  ...
  function(req, res) {
    ...
    params.to = parseInt(params.to, 10);
    if (params.type == null || isNaN(params.from) || isNaN(params.to)) {
      return req.send(400);
    }
    model.db.relate(params.from, params.type, params.to, req.body,
    function(err, rel) {
      if (err) res.send(err.message || err, 500);
      else res.json(rel);
    });
  ...
  controller.get(join(root, ':id', ':type', ':direction?'), 'rel:read');
  controller.post(join(root, ':from', ':type', ':to'), 'rel:create');
